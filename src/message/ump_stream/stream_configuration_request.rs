...
mod tests {
    ...
    fn builder() {
        assert_eq!(
            StreamConfigurationRequestMessage::builder()
                .protocol(0x2)
                .receive_jr_timestamps(true)
                .send_jr_timestamps(true)
                .build(),
            Ok(StreamConfigurationRequestMessage::Owned(
                StreamConfigurationRequestOwned([0xF005_0203, 0x0, 0x0, 0x0,])
            )),
        );
    }
    ...
    fn protocol() {
        assert_eq!(
            StreamConfigurationRequestMessage::from_data(&[0xF005_0203])
                .unwrap()
                .protocol(),
            0x2
        );
