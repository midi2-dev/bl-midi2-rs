...
impl<'a> Sysex8MessageGroupBuilder<'a> {
    ...
    fn grow(&mut self) {
        ...
        {
            let mut builder = Sysex8Message::builder(&mut self.buffer[4*self.size..4*(self.size + 1)]);
        ...
        if self.size != 0 {
            let mut prev_builder = Sysex8MessageBuilder(Ok(&mut self.buffer[4*(self.size - 1)..4*self.size]));
            match self.size {
                1 => {
